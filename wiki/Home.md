...
**LogitechGHub** is the device-specific fallback for the Logitech PRO X family (PRO X, PRO X 2, PRO X Wireless), whose Windows endpoint can remain `Active` while the physical headset is off. AutoSwitch uses G HUB's unofficial local WebSocket as the signal.
